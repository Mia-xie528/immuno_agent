...
LLM_API_KEY = "sk-3d1fe616d7ed49b2b934d77fcce19bab"
# 模型名称必须与所使用的平台一致。
...
def create_llm_client() -> OpenAI:
    ...
    if normalize_text(LLM_API_KEY) in placeholder_values:
        raise RuntimeError(
            "尚未配置 LLM_API_KEY。请在配置区填写密钥，"
            "或设置环境变量 LLM_API_KEY。"
        )
    ...
    return OpenAI(
        ...
        max_retries=0,  # 由本脚本统一控制重试，避免双重重试。
    )
